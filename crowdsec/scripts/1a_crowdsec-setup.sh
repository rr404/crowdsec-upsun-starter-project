...
setup_api() {
	if $BASE/cscli -c "$CONFIG_FILE" capi status 2>/dev/null; then
		echo "CAPI already configured"
	else
		echo "Configuring CAPI"
		$BASE/cscli -c "$CONFIG_FILE" capi register
	fi

	if $BASE/cscli -c "$CONFIG_FILE" lapi status 2>/dev/null; then
		echo "LAPI already configured"
	else
		echo "Configuring LAPI"
		$BASE/cscli -c "$CONFIG_FILE" lapi register
